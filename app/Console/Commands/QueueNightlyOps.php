...
use App\Jobs\RunNightlyOpsJob;
use App\Support\CronExecutionLogger;
use Illuminate\Console\Command;
use Illuminate\Support\Facades\Schema;
use Throwable;

class QueueNightlyOps extends Command
{
    use ResolvesActiveTenants;

    protected $signature = 'ops:queue-nightly-closing
    ...
    protected $description = 'Dispatch ops:nightly-closing ke queue worker';
    ...
    public function handle(): int
    {
        ...
        foreach ($tenantIds as $tenantId) {
            try {
                ...
                $cronLogger->record(
                    (int) $tenantId,
                    'nightly_closing',
                    'ops:queue-nightly-closing',
                    'queued',
                    'Dispatch closing malam ke queue worker.',
                    [
                        'mode' => 'queued',
                ...
                $cronLogger->record(
                    ...
                    'ops:queue-nightly-closing',
                    'failed',
                    'Gagal dispatch closing malam: ' . substr($e->getMessage(), 0, 160),
                    [
                        'mode' => 'queued',
                        'connection' => $queueConnection,
                        'queue' => $queue,
        ...
        $this->info("Dispatch nightly ops selesai: queued={$successCount}, failed={$failedCount}");
